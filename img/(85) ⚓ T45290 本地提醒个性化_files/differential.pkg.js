...
highlighted;var
linked=[];var
isMac=navigator.platform.indexOf('Mac')>-1;var
signalKey=isMac?91:17;function
isSignalkey(event){return isMac?event.getRawEvent().metaKey:event.getRawEvent().ctrlKey;}var
...
link(element,lang){JX.DOM.alterClass(element,'repository-crossreference',true);linked.push(element);JX.DOM.listen(element,['mouseover','mouseout','click'],'tag:span',function(e){if(e.getType()==='mouseout'){unhighlight();return;}if(!isSignalkey(e)){return;}var
target=e.getTarget();try{if(JX.DOM.findAbove(target,'div','differential-inline-comment')){return;}}catch(ex){}if(e.getType()==='mouseover'){while(target!==document.body){if(JX.DOM.isNode(target,'span')&&(target.className
in
class_map)){highlighted=target;JX.DOM.alterClass(highlighted,classHighlight,true);break;}target=target.parentNode;}}else
if(e.getType()==='click'){openSearch(target,lang);}});}function
unhighlight(){highlighted&&JX.DOM.alterClass(highlighted,classHighlight,false);highlighted=null;}function
openSearch(target,lang){var
symbol=target.textContent||target.innerText;var
query={lang:lang,repositories:config.repositories.join(','),jump:true};var
...
uri=JX.$U('/diffusion/symbol/'+symbol+'/');uri.addQueryParams(query);window.open(uri);}function
linkAll(){var
...
lang=blocks[i].getAttribute('data-code-lang');link(blocks[i],lang);}}}if(config.container){link(JX.$(config.container),config.lang);}else
if(config.section){linkAll(JX.$(config.section));}JX.Stratcom.listen('differential-preview-update',null,function(e){linkAll(e.getData().container);});JX.Stratcom.listen(['keydown','keyup'],null,function(e){if(e.getRawEvent().keyCode!==signalKey){return;}setCursorMode(e.getType()==='keydown');if(!statics.active){unhighlight();}});JX.Stratcom.listen('blur',null,function(e){if(e.getTarget()){return;}unhighlight();setCursorMode(false);});function
setCursorMode(active){statics.active=active;linked.forEach(function(element){JX.DOM.alterClass(element,classMouseCursor,statics.active);});}});JX.behavior('load-blame',function(config){new
JX.Request(location.href,function(response){JX.DOM.setContent(JX.$(config.id),JX.$H(response));}).send();});JX.behavior('differential-user-select',function(){var
...
rows=JX.DOM.scry(this._node,'tr');for(var
ii=0;ii<rows.length;ii++){var
row=rows[ii];if(this._getRowType(row)!='comment'){continue;}this.getInlineForRow(row);}},toggleVisibility:function(){this._visible=!this._visible;var
...
inline=this._getInlineForEvent(e);inline.setCollapsed(is_collapse);},_onresize:function(){this._redrawFocus();this._redrawSelection();this._redrawHover();this._bannerChangeset=null;this._redrawBanner();},_onscroll:function(){this._redrawBanner();},_onselect:function(e){if(e.getTarget()!==e.getNode('differential-inline-header')){return;}var
...
changeset=this._getVisibleChangeset();if(this._bannerChangeset===changeset){return;}this._bannerChangeset=changeset;if(!changeset){JX.DOM.remove(node);return;}var
