...
workflow=new
JX.Workflow(form.getAttribute('action'),{});workflow._form=form;workflow.setDataWithListOfPairs(pairs);workflow.setMethod(form.getAttribute('method'));workflow.listen('finally',function(){for(var
...
e=active.invoke('submit',{form:form,data:data});if(!e.getStopped()){active.setURI(form.getAttribute('action')||active.getURI()).setDataWithListOfPairs(data).start();}},_getActiveWorkflow:function(){var
...
self=JX.Workflow;if(!self._resizing){return;}self._resizing=false;}},members:{_root:null,_pushed:false,_data:null,_form:null,_paused:0,_nextCallback:null,getSourceForm:function(){return this._form;},pause:function(){this._paused++;return this;},resume:function(){if(!this._paused){JX.$E('Resuming a workflow which is not paused!');}this._paused--;if(!this._paused){var
...
k
in
t.map){JX.DOM.alterClass(JX.$(k),t.map[k],t.state);}});JX.Stratcom.listen(['click'],'jx-toggle-class',function(e){e.kill();});return true;}statics.install=statics.install||install();});JX.behavior('lightbox-attachments',function(config){var
lightbox=null;var
prev=null;var
next=null;var
shown=false;var
downloadForm=JX.$H(config.downloadForm).getFragment().firstChild;var
lightbox_id=config.lightbox_id;function
_toggleComment(e){e.kill();shown=!shown;JX.DOM.alterClass(JX.$(lightbox_id),'comment-panel-open',shown);}function
...
loadLightBox(e){if(!e.isNormalClick()){return;}e.kill();var
...
nameElement=JX.$N('div',{className:'attachment-name'},target_data.name);img=JX.$N('a',{className:'lightbox-icon-frame',sigil:'lightbox-download-submit',href:'#',},[imgIcon,nameElement]);}var
...
commentClass=(shown)?'comment-panel-open':'';lightbox=JX.$N('div',{className:'lightbox-attachment '+commentClass,sigil:'lightbox-attachment',id:lightbox_id},[imgFrame,commentFrame]);var
...
statusHTML=JX.$N('div',{className:'lightbox-status'},[statusSpan,closeButton,commentButton,downloadSpan]);JX.DOM.appendContent(lightbox,statusHTML);JX.DOM.listen(closeButton,'click',null,closeLightBox);var
...
l_icon=new
JX.PHUIXIconView().setIcon('fa-angle-left').setColor('lightgreytext').getNode();rightIcon=JX.$N('a',{className:'lightbox-left',href:'#'},l_icon);JX.DOM.listen(rightIcon,'click',null,JX.bind(null,loadAnotherLightBox,prev));}JX.DOM.appendContent(lightbox,rightIcon);JX.DOM.alterClass(document.body,'lightbox-attached',true);JX.Mask.show('jx-dark-mask');downloadForm.action=target_data.dUri;downloadSpan.appendChild(downloadForm);document.body.appendChild(lightbox);if(img_uri){JX.Busy.start();img.onload=function(){JX.DOM.alterClass(img,'loading',false);JX.Busy.done();};img.src=img_uri;}loadComments(target_data.phid);}function
...
loadAnotherLightBox(el,e){if(!el){return;}e.prevent();closeLightBox(e);el.click();}JX.DOM.listen(JX.$('main-page-frame'),'click',['lightboxable'],loadLightBox);JX.Stratcom.listen('keydown',null,lightBoxHandleKeyDown);JX.Stratcom.listen('click','lightbox-image-frame',function(e){if(!lightbox){return;}if(e.getTarget()!=e.getNode('lightbox-image-frame')){return;}closeLightBox(e);e.kill();});JX.Stratcom.listen('click','lightbox-comment',_toggleComment);var
...
_startDownload=function(e){e.kill();var
form=JX.$('lightbox-download-form');form.submit();};var
_startPageDownload=function(e){e.kill();var
form=e.getNode('tag:form');form.submit();};JX.Stratcom.listen('click','lightbox-download-submit',_startDownload);JX.Stratcom.listen('click','embed-download-form',_startPageDownload);});JX.install('Busy',{statics:{_depth:0,start:function(){var
...
self=JX.Aphlict;if(!self._instance){return null;}return self._instance;}}});JX.install('Notification',{events:['activate','close'],members:{_container:null,_visible:false,_hideTimer:null,_duration:12000,_asDesktop:false,_key:null,_title:null,_body:null,_href:null,_icon:null,show:function(){var
...
response=e.getData();if(!response.showAnyNotification){return;}new
JX.Notification().setContent(JX.$H(response.content)).setKey(response.primaryObjectPHID).setShowAsDesktopNotification(response.showDesktopNotification).setTitle(response.title).setBody(response.body).setHref(response.href).setIcon(response.icon).show();if((response.primaryObjectPHID
in
...
ii;var
type_order=['jump','apps','proj','user','repo','symb','misc'];var
...
collapsed=config.collapsed;JX.Stratcom.listen('differential-filetree-toggle',null,function(){collapsed=!collapsed;JX.DOM.alterClass(main,'has-local-nav',!collapsed);JX.DOM.alterClass(main,'has-drag-nav',!collapsed);JX.DOM.alterClass(main,'has-closed-nav',collapsed);resetdrag();new
