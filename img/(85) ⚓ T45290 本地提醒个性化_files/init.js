...
ii=0;ii<holding_queues[name].length;ii++){fn.apply(null,holding_queues[name][ii]);}holding_queues[name]={};};makeHoldingQueue('install');makeHoldingQueue('behavior');makeHoldingQueue('install-init');window.__DEV__=window.__DEV__||0;var
loaded=false;var
onload=[];var
master_event_queue=[];var
root=document.documentElement;var
has_add_event_listener=!!root.addEventListener;JX.__rawEventQueue=function(what){master_event_queue.push(what);var
Stratcom=JX['Stratcom'];if(Stratcom&&Stratcom.ready){var
local_queue=master_event_queue;master_event_queue=[];for(var
ii=0;ii<local_queue.length;++ii){var
evt=local_queue[ii];try{var
test=evt.type;}catch(x){continue;}if(!loaded&&evt.type=='domready'){document.body&&(document.body.id='');loaded=true;for(var
