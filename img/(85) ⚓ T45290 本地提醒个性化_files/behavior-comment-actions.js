JX.behavior('comment-actions',function(config){var
...
get_data(){var
data=JX.DOM.convertFormToDictionary(form_node);data.__preview__=1;data[input_node.name]=serialize_actions();return data;}function
restore_draft_actions(drafts){var
draft;var
option;var
control;for(var
ii=0;ii<drafts.length;ii++){draft=drafts[ii];option=find_option(draft);if(!option){continue;}control=add_row(option);}}function
...
preview_root=JX.$(config.timelineID);JX.DOM.setContent(preview_root,[JX.$H(response.xactions.join('')),JX.$H(response.previewContent)]);JX.DOM.show(panel);var
...
remove=JX.$N('a',{href:'#'},icon.getNode());var
control=new
...
panel=JX.$(config.panelID);if(new_device=='desktop'){request.setRateLimit(500);if(old_device!=new_device){force_preview();}}else{request.setRateLimit(10000);JX.DOM.hide(panel);}old_device=new_device;};ondevicechange();JX.Stratcom.listen('phabricator-device-change',null,ondevicechange);}restore_draft_actions(config.drafts||[]);});